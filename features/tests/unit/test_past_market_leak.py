...
def test_feature_version_servability():
    ...
    # Feature 091: lgbm-064 (features-018) must stay servable under features-021, and
    # only with its OWN trained hash — a wrong hash still fails closed.
    _pin018 = COMPATIBLE_PRIOR_FEATURE_VERSIONS["features-021"]["features-018"]
    assert is_feature_version_servable("features-018", _pin018, "features-021")
    assert not is_feature_version_servable("features-018", "deadbeef", "features-021")
    # compat is NOT transitive: 017 does not ride through 018 into 020.
    assert not is_feature_version_servable("features-017", _pin017, "features-021")
