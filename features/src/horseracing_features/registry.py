...
REGISTRY: dict[str, FeatureMeta] = {
    ...
    # winner-NLL CI upper bound crossed zero each time — not significant). The past-market rank/
    # residual/conditioned axes are ~redundant with F02 (log(q·N)) + 058 (raw rank). The
    # FEATURE_VERSION bump is REVERTED (027/062 precedent: a rejected bump would put lgbm-064-f02acc
    # onto the compat path for no gain). pm_rank_robust/pm_expectation_residual/pm_conditioned.py +
    # their unit tests are preserved (unwired) as the documented negative result.
