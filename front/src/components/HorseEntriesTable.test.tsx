...
describe("HorseEntriesTable profile links (029)", () => {
  it("links canonical horse/jockey names but not nk: surrogates", () => {
    renderWithProviders(<HorseEntriesTable entries={entries} predictions={[]} />);
    // canonical ids -> links to the profiles
    ...
    // surrogates -> plain text, no link
    expect(screen.queryByRole("link", { name: "サロゲート馬" })).toBeNull();
    expect(screen.queryByRole("link", { name: "サロゲート騎手" })).toBeNull();
    expect(screen.getByText("サロゲート騎手")).toBeInTheDocument();
  });
});
