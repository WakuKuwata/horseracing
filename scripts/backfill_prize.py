...
One process, one fetcher, one thread, sequential — so `HttpFetcher`'s per-instance interval IS the
real interval here (the per-fetcher/per-host limiter problems tracked in 093 are about CONCURRENT
streams). At the default 60s that is ~2,946 requests ≈ 49 hours, so:

  * STOP THE OPS WORKER FIRST, or the daily job is a second stream and the source sees double.
