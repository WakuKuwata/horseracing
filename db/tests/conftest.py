...
A single session-scoped PostgreSQL 16 testcontainer is started, migrated to head
once, and every test runs against it with all tables truncated between tests.
"""
...
def _truncate_between_tests(request):
    # Only integration tests touch the DB; unit tests must not start the container.
...
def alembic_cfg(database_url: str) -> Config:
    """Alembic Config wired to the testcontainer, for migration round-trip tests."""
