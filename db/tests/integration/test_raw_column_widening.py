"""Feature 055: migration 0010 — nullable raw-column widening.

The added fields cover first_3f, prize, owner, breeder, and bloodline names.
"""
...
def test_new_columns_exist_nullable_and_roundtrip(session):
    ...
    session.add(
        Race(race_id="202601010102", race_number=2, race_date=datetime.date(2026, 1, 1))
    )  # prize NULL ok
    session.add(
        Horse(
            horse_id="H055A",
    ...
    session.add(
        RaceResult(
            ...
            horse_id="H055A",
            finish_order=1,
            first_3f=Decimal("35.6"),
            last_3f=Decimal("34.4"),
        )
    )
    ...
    assert (
        session.scalars(select(RaceResult).where(RaceResult.horse_id == "H055B")).one().first_3f
        is None
    )
