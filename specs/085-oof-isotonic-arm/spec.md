...
## 14. §12/§13 の前提の一部が解消(2026-08-18・append-only)

§12.2 と §13.1 で「前向き窓は年単位 fold のため隔離できない」と書いたが、これは
`splits.expanding_folds` に `valid_from`(**採点側だけ**を日付で絞る)を足して解消した。
学習側は年単位のまま据え置いたので、**Jan-1 始まりの既存 config は 1 件も数値が動かない**
(068/069/070/073/088/090/091 はすべて Jan-1 始まり)。

これは §13 の昇格判断を取り消すものではない。昇格は既に実行済みで、§13.1 が挙げた理由は
隔離不能のほかに 3 つある(選択の膨らみが効果量の 1/4.6・2026 単独でもゼロを切る・
外したときの損失が小さく可逆)。**ただし「隔離できないから待っても無駄」という論拠は
もう使えない**ので、記録として残す。

以後の prospective 設計では日付境界の窓が正しく効く。
