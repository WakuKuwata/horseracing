...
**Status**: 実装完了・**案D デプロイ採用(精度最優先モデル lgbm-058-acc を非 active 登録・製品セレクタ read-only 表示)**
...
**serving 互換(T013)の決定=案D(低リスク)**: serving `load_serving_model` は `feature_hash(model_input_features())`(グローバル現行特徴セット)を全 servable モデルに要求するため、**features-015 を本番 registry に bump すると現 active lgbm-057(features-014)が fail-closed** になる。codex second opinion + 分析の収束で、**本番 registry/serving は features-014 のまま据え置き**、lgbm-058-acc は worktree(features-015)から予測を **backfill で永続化**し、API read 経路(persisted 予測を読むだけ=特徴再ビルドしない、`api/selection.py::select_prediction_run`)経由で 057 のセレクタに表示する方式を採用。**default serving・SC-005 完全不変**。代償=accuracy モデルの live 新規予測は features-015 serving を要するため backfill 済みレースのみ切替表示(将来 live 予測が本番必須になれば案C'=per-model feature_cols hash + 互換 allowlist + 旧列 byte parity gate を別 feature で実装)。**features-015 コードは 058 ブランチに保全し main serving へはマージしない**(マージ=registry bump=lgbm-057 fail-closed のため)。
