...
- **FR-015** (US5, 追補): システムは馬の db.netkeiba.com から**識別・血統属性のみ** (性別 / 生年 / 父・母・母父の ID・名) を抽出し、`horses` の **NULL 列のみ**に補完できなければならない (MUST)。識別 (馬名/性別/生年) は本体ページ `/horse/{id}/` から、血統は専用ページ `/horse/ped/{id}/` (サーバ描画 `blood_table`) から取得する (本体ページの血統は JS 描画のため、FR-013 整合で 2 ページ静的取得)。既存値 (JRA-VAN 由来含む) を上書きしてはならない (MUST NOT)。血統馬 ID も `id_mappings` 経由で解決し推測結合しない (MUST NOT)。`ingestion_jobs` に job_type='horse_profile' で記録する (MUST)。騎手・調教師は補完すべき列が無く対象外とする。
- **FR-016b** (US5, 改訂 2026-06-28): 補完は **entries 取り込み後に当該レースの surrogate 馬 (識別/血統が NULL のもの) を対象に自動実行する** (既定 ON; `scrape_entries(complete_profiles_after=...)` / CLI `--no-complete-profiles` で無効化可)。新馬・未登録馬が別コマンド無しで血統まで揃う。**決定性・隔離**: 補完は entries ジョブが確定 (commit) した後に独立ジョブとして走り、parse/upsert の書き込み経路には入らない (codex 指摘の遅延フェッチ回避)。1 馬のページ取得失敗は horse_profile ジョブに error 計上されるのみで、entries を巻き戻さず失敗もさせない (MUST)。明示起動 (`complete-profiles`、全 surrogate 馬対象) も従来どおり可。当初の opt-in 既定 (FR-015 旧文) からの意図的変更。
