...
| **INV-W7** | `features-021` の共有列(既存 137 列)は `features-018` とバイト一致する | 実 DB で一度きり実測(058/061/069 と同型) |
| **INV-W8** | `source_fingerprint` が `features-018` から変化しない | 実 DB で実測。新ソース列を読まないことの機械的確認 |
| **INV-W9** | active `lgbm-064-f02acc` の予測が `features-021` の下でバイト不変 | 実 DB E2E(compat 経路)。1 頭 1 ビットも変わらないこと |
