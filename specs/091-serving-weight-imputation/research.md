...
## D14: 診断アーム m=1.0 は harness の整合チェックに引っかかる — 例外を「設計上の性質」に限定して開ける

**実行して初めて分かった**: m=1.0(全レース mask で学習)のアームを回すと、`evaluate_regimes` の
fail-closed が発火して評価そのものが止まった。

```
PairedContractError: serving regime had NO effect on ['candidate']:
winner NLL is bit-identical to full-info.
```

このチェックは正しく働いている。**m=1.0 のモデルは学習中に当日体重を一度も見ないので、
予測時に体重を mask しても何も変わらない**。だからチェックの言うとおり「その arm は
`weight` / `weight_diff` / `carried_weight_ratio` を一切読んでいない」。

問題は、それが**この対照アームでは故障ではなく測ろうとしている性質そのもの**である点にある。
m=1.0 の問いは「当日体重を設計から捨てたらどうなるか」であり、regime 不変であることは
答えの一部であって、答えが得られない理由ではない。

### 開けた穴とその限定

- **verdict 適格な artifact では従来どおり厳格**。緩和は `artifact_kind != full_walk_forward` に限る
- **候補アームだけが inert のときに限る**。active アームは `prev_weight` を持たず体重列を読むので、
  mask が効いていれば必ず影響を受ける。**両アームが inert なら配線の故障**なので従来どおり落とす
- 緩和が働いたことは `notes.candidate_is_regime_invariant` と説明文で artifact に**明示**する。
  そうしないと読み手は「serving regime の比較」として読んでしまうが、片方の arm はその regime を
  一度も経験していない

つまり「故障」と「設計上の性質」は**観測で区別できる**(inert なのが片側か両側か)ので、
その区別を規則にした。区別できないなら緩和すべきではなかった。

**教訓**: 整合チェックは「あってはならないこと」を書くが、対照実験は往々にして
「通常あってはならないこと」を意図的に起こす。チェックを緩めるのではなく、
**緩和条件を観測可能な述語として書けるかどうか**を判断基準にした。

## D15: T052 block 幅感度 — 結論は推定量の選択に依存しない

診断(ゲートに AND しない)。凍結した primary は `race_day_cluster_bootstrap_ci_v1`。

| block | serving diff | 95% CI | blocks |
|---|---|---|---|
| **race_day(primary)** | −0.010539 | [−0.012835, −0.008115] | 602 |
| 2d | −0.010539 | [−0.012822, −0.008244] | 301 |
| 3d | −0.010539 | [−0.012753, −0.008375] | 201 |
| 4d | −0.010539 | [−0.013055, −0.008074] | 151 |
| week | −0.010539 | [−0.012818, −0.008252] | 292 |

CI 上限の振れ幅は −0.008074〜−0.008375(3.7%)で、**どの幅でもゼロを大きく下回る**。
開催日クラスタという選択が結論を作っているわけではない。m=1.0 の対照アームでも同じ(振れ 1.5%)。

**この感度をゲートに AND しない理由**は変わらない: ゼロを跨ぐか跨がないかの境目にある候補で、
幅を選べる余地を残すこと自体が事後の推定量選択になる。今回は全幅で同じ答えなので**選ぶ余地が
そもそも無い**ことが確認できた、という位置づけである。

## D16: 「決定論的に測った」は事実ではなかった — 生きた DB を読んでいる

`num_threads: 1` の未配線を再現性ばらつきの原因と帰属したが、**それは原因の全部ではなかった**。
配線後の再実行でも 5.3e-5 ずれ(元の 8.5e-5 と同じ桁)。

調べると **評価窓 267,744 行のうち 12,816 行(4.8%)が最初の実行以降に書き換えられていた**
(すべて 2026 年 = 日次取込の再 upsert)。憲法 V により行は最新値 1 つしか持たないので、
**値が変わったのか `updated_at` だけが動いたのかは遡及確認できない**。

→ 凍結設定の `tolerance: 1e-9` は現在の実行方式では**原理的に検証できない**。

**教訓**: 再現性の宣言をする前に、その推定量の**入力が固定されているか**を確認する。
LightGBM の決定性(スレッド・seed)は入力が同じ場合の話であり、入力が動いていればそこは論点ですらない。
最初に thread に飛びついたのは、**測定器の内部を疑って測定対象の供給元を疑わなかった**からである。

**残る作業(本 spec の外)**: `paired-eval` に `--use-materialized` が無い(serving/training にはある)。
fingerprint で固定された parquet を読ませれば評価は厳密に再現可能になる。088 の D12 が
「DB は実装中も動く」として in-process 二重ビルドで回避した罠の、評価側での再来である。
