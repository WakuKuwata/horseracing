...
contracts(feature-columns INV-EM1..EM8・adoption-gate) / gate-config.json(凍結 hash 6f76bf15…=analyze 2 周目反映後の再事前登録値)
...
`verdict.status ∈ {ADOPT, REJECT, NO_DECISION(実行不能または事前登録 sufficiency 未達のみ)}` を持ち、quickstart §2 の出力形と一致。
...
- [X] T020 [P] [US2] `eval/tests/unit/test_paired_report_diffs_by_day.py`: T003 の露出フィールドで pooled CI が単窓 CI と整合する(1 窓だけ渡せば既存 `bootstrap_ci` と一致)ことを固定

### Implementation for User Story 2

- [X] T021 [US2] `scripts/097_simulated_supply_gate.py` を新規作成。`assert_confirmatory(cfg, expected_hash, eval_window)` を冒頭で通し、3 カットオフを順次: (1) `s.rollback()` (2) マスク UPDATE(未 commit) (3) **対称性 assert**: マスク前後で `load_eval_races` の race_id 集合・各レースの started 集合・winner が同一(破れたら即 fail) (3b) **マスク provenance assert**(codex tasks Q2・analyze C6 で実装可能な形に是正): `_ensure_data()` の**直前**に、マスク済みセッション `s` で射影クエリ 1 本 `(race_id, horse_id, race_date, distance, first_3f)` を取り `frame_projection_hash` を記録、かつ cutoff 以降∧距離≠1200m の first_3f 非 NULL が **0 行**であることを assert(`supply_mask.provenance_violations`)。hash が証明するのは「matrix 構築直前のセッション状態」であり、matrix 自体は (3c) の単一構築で同一性を担保する。両アームとも `use_materialized=False` を assert (3c) **単一構築**(analyze P3/C6): マスクを当てたセッション `s` で `LightGBMPredictor(s, ...)._ensure_data()` を **1 回だけ**呼んで `TrainingMatrix` を作り、両アームの `CalibSplitFactory._shared` に同一オブジェクトを注入する(fold 内での再構築禁止・`is` で同一性 assert)。「1 回だけ」の対象は matrix 構築であり、(3b) の射影クエリは別に 1 本走る。`eval_window` は `cfg["eval_window"]` をそのまま渡す(窓照合は恒真=analyze C10。実効検査は `scored_windows ⊂ envelope` の assert)。**窓ごとのレース集合**(analyze C8): `paired_eval` に `valid_to` は無いので、各窓で `load_eval_races(s, end_date=window_to)` とし `first_valid_year=window_from.year`・`valid_from=window_from` で**単一 fold = 当該窓**にする(envelope 全体を渡すと 2020 窓が 2020-2024 を採点し washout 違反) (4) 候補 factory = `CalibSplitFactory(ModelRecipe(pl_topk, calibration none, params n_estimators=900, weight_mask 0.5/20260810, seed 42), n_oof_blocks=8, isotonic)` / 基準 = 同レシピ + `drop_features=FEATURE_GROUPS の early_mid_pace 展開` (5) `paired_eval(... first_valid_year=窓年, valid_from=窓始, subgroups=False, num_threads=1)` (6) `diffs_by_day` を回収 (7) `s.rollback()`。採点窓は gate-config の `scored_windows` を読む(ハードコード禁止)。**出力規律**(codex tasks Q1b): primary・guard1・guard2 の全成分が揃うまで効果の数値を stdout にも JSON にも出さない(進捗は「cutoff X: mask OK / symmetry OK / provenance OK / build OK / fit OK」のみ(実行順=(2)(3)(3b)(3c)(4-5)))
- [X] T022 [US2] 同 driver: 3 窓の `diffs_by_day` を union(互いに素を assert)→ `race_day_cluster_bootstrap_ci_v1` → `inflate_for_seed_noise(sd_fold, n_folds=3)` → `recent_window_guard(pooled_diffs_by_day, cfg=cfg, max_date=2024-12-31)`(gate-config `recent_guard` の凍結値。暦日で 3y 窓=2022/2024 の採点日・5y 窓=3 窓全部) → `evaluate_core_gate(diff, ci_low, ci_high(総), recent=その結果, top2/top3 は n_races 加重平均・ECE も n_races 加重平均(近似と明記=gate-config `pooling`), cfg)` で v4 標準式を適用。**sufficiency**(analyze C1): pooled n_days ≥ `min_eval_days`(300・実測 323)でなければ NO_DECISION。窓ごとの `PairedReport.decision` は参考値で転記しない。**新しい判定式を書かない**
- [X] T023 [US2] 同 driver: guard 1(full-info)— マスク無しで同 3 窓を同手順で実行し pooled diff に `three_way(ci_low, ci_high, margin=0.003)` → FAIL なら guard1=False。guard 2 — 実窓 2025-10-11..最新(マスク無し・first_valid_year=2025・valid_from=2025-10-11)の paired diff に `ci_low > 0.005` で FAIL。**guard の CI は標本 CI(seed inflate 無し)**=v4 の evidence-of-harm 既定(`recent_window_guard`・subgroup `three_way`)と同じ。CI を広げるほど FAIL しにくくなる向きなので事前に 1 bit 固定(analyze 3 周目 A1)
- [X] T024 [US2] 同 driver: verdict JSON を `specs/097-early-mid-pace/verdict.json` に書く: `evaluation_contract_version="v4"`・`artifact_kind="counterfactual_supply_simulation"`(codex tasks Q1d: full_walk_forward だと `evaluate_promotion` がこれで ACTIVE 昇格を通す構造的な穴 → 専用種別で昇格ゲートから弾く。列採用の適格性は別キー `feature_adoption_eligible=True`)・`eligible_for_verdict=False`(モデル昇格には使えない)・`evidence_regime="masked_pseudo_supply_death"`・`model_training_regime="real_unmasked"`(ADOPT 後に実データで学習する登録モデルのレジーム=証拠と異なることを verdict 側に明示・analyze I5)・gate_config_hash・両アーム recipe_hash・各カットオフの race_id_set_hash と provenance hash・マスク定義・採点窓・pooled point/CI(標本・総)・pooled n_days と sufficiency・guard 2 本の詳細(guard 2 は `window_to`=実行時 DB 最新日と race_set_hash を記録=analyze C11)・**`verdict` は RegimeReport 準拠のオブジェクト** `{"status": "ADOPT|REJECT|NO_DECISION", "adopt": bool, "formula": "primary_pooled AND guard1 AND guard2", "decision_reason": {...}}`(文字列にすると `normalize_verdict` が `.get` で落ちる=analyze C5)・「反実仮想頑健性(counterfactual robustness)として報告」の注記
- [X] T025 [US2] smoke(配線のみ): gate-config `smoke` 節の**事前登録外カットオフ 2016-01-01・採点窓 2017**(codex tasks Q1a: 実カットオフの smoke は確認データの中間閲覧になる)・rounds=50・blocks=2・b=200 で driver を通し、対称性・provenance・JSON 形を確認。**効果の数値は stdout/JSON とも非表示**(`redact_effect_numbers`)。`artifact_kind="smoke"` で `out/` に出す(追跡しない)
- [X] T026 [US2] 本実行: quickstart §2 のコマンドで nohup 実行(推定 ~4.5h・実績比)。完了後 `verdict.json` をコミット。**個別カットオフの数値を見て解釈を変えない**(pooled が正本)
- [X] T027 [US2] 結果を `spec.md` 末尾の「実測結果」節に転記(primary pooled・guard1・guard2・verdict・限界 D10 の再掲)
...
- [X] T028 [US3] `registry.py` の FEATURE_VERSION を features-021 に戻し、2 列の REGISTRY/FEATURE_GROUPS/compat pin エントリを削除。`materialize.py` の leaf block と結線を削除。`test_registry_features022.py` を削除。front/admin のラベル 2 行を削除(表示専用なので残しても害は無いが、model-input 列でないものにラベルを残さない規律)
- [X] T029 [US3] 実 DB で `predict --race-id <T018 と同じレース>` を再実行し、revert 後の予測が T018 の値と 18 頭バイト一致(SC-005)。`evidence-preflight.md` に追記
- [X] T030 [US3] `early_mid_pace_features.py` + `test_early_mid_pace_features.py` + `test_early_mid_pace_leak.py` を**非結線で保全**(062/070/090 同型)。テストは `build_early_mid_pace_features` 直呼びで緑であることを確認。モジュール docstring 冒頭に「REJECT(日付・verdict.json 参照)・非結線保全」を追記
- [X] T031 [US3] `features materialize` を features-021 で再実行し parquet を戻す(manifest の feature_version 確認)

### ADOPT 分岐(非該当 — verdict は REJECT)

- [ ] T032 [US3] 実データ(マスク無し)で候補モデルを学習・登録: `register-arm-e --model-version lgbm-097-emp --n-estimators 900 --weight-mask-rate 0.5 --weight-mask-seed 20260810 --n-oof-blocks 8 --seed 42 --artifacts-dir <絶対パス>`(**gate-config `arms.recipe` と一致させる**=analyze C3: `--weight-mask-*` を省くと None=マスク無しで学習され、active とも gate とも別レシピの候補が無音で登録される。weights_uri 相対パス事故の回避で `--artifacts-dir` は絶対パス。`--verdict` 引数は存在しない=register-arm-e は常に CANDIDATE 登録)。続けて `promote-model --model-version lgbm-097-emp --verdict ../specs/097-early-mid-pace/verdict.json`(**--apply 無し=dry-run**)を実行し、**`error: v3 verdict が昇格要件を満たさないため override が必要: verdict_artifact_not_eligible` + exit 1 が期待結果**(失敗ではない=構造的拒否の証明)であることを `evidence-preflight.md` に貼る。証拠と学習レジームの違い(`evidence_regime=masked_pseudo_supply_death` / `model_training_regime=real_unmasked`)は verdict.json 側に記録済み(T024)。**昇格しない**(contracts/adoption-gate: 標準窓非劣化+本 verdict+prospective の 3 点セットで別段)
...
- [X] T034 [P] `CLAUDE.md` の SPECKIT 区間を**手動 Edit**で「実装完了・verdict」に更新(hook 実行禁止)
- [X] T035 [P] memory: `first3f-per-horse-unavailable` に 097 の verdict と数値を追記。ADOPT なら `feature-097-early-mid-adopt` を新規
- [X] T036 全パッケージ(features/eval/training/serving/front/admin)のテストと ruff を実行し緑を確認。`api` の chaos p95 既知事象は対象外と明記
- [X] T037 コミット・push。commit 本文に pooled 数値・guard・verdict・限界(D10)を残す
