...
export const FEATURE_LABELS: Record<string, FeatureLabel> = {
  ...
  // --- corner trajectory (041, as-of) ---
  asof_late_gain_avg: { label: "直線での順位上げ（平均）" },
  asof_late_gain_best: { label: "直線での順位上げ（最良）" },
  asof_early_pos_avg: { label: "序盤の位置取り（平均）" },
  asof_mid_move_avg: { label: "道中の捲り（平均）" },
  // --- first-3F pace (056, as-of) ---
  ...
  asof_breeder_win_rate: { label: "生産者 勝率" },
  // --- race level / prize (056) ---
  prize_money_log: { label: "レース賞金（格）" },
  asof_prize_avg: { label: "過去走レースの賞金水準（平均）" },
  prize_rel: { label: "賞金水準の変化（昇降級度合い）" },
  // --- sire line (056) ---
  sire_line: { label: "父系統" },
  damsire_line: { label: "母父系統" },
  // --- relative ability vs field (059) ---
  win_rate_vs_field: { label: "勝率（出走馬平均比）" },
  recent_win_rate_vs_field: { label: "直近勝率（出走馬平均比）" },
  place_rate_vs_field: { label: "連対率（出走馬平均比）" },
  show_rate_vs_field: { label: "複勝率（出走馬平均比）" },
  dist_band_win_rate_vs_field: { label: "距離帯別勝率（出走馬平均比）" },
  surface_win_rate_vs_field: { label: "芝ダ別勝率（出走馬平均比）" },
  rel_time_avg_vs_field: { label: "走破時計（出走馬平均比）" },
  rel_last3f_avg_vs_field: { label: "上がり3F（出走馬平均比）" },
  finish_diff_best_vs_field: { label: "着差最良（出走馬平均比）" },
  jockey_win_rate_vs_field: { label: "騎手勝率（出走馬平均比）" },
  trainer_win_rate_vs_field: { label: "調教師勝率（出走馬平均比）" },
  win_rate_field_rank: { label: "勝率のレース内順位（百分位）" },
  rel_time_avg_field_rank: { label: "走破時計のレース内順位（百分位）" },
  // --- past market rank (058, as-of; past-race closing popularity, never today's odds) ---
  asof_mkt_rank_avg: { label: "過去走の人気順位（平均）" },
  asof_mkt_rank_norm_avg: { label: "過去走の人気（頭数正規化・平均）" },
  asof_mkt_rank_best: { label: "過去走の人気順位（最良）" },
  asof_beat_mkt_avg: { label: "人気と着順の差（平均）" },
  // --- speed figure (061, as-of) ---
  asof_spdfig_avg: { label: "スピード指数（平均）" },
  asof_spdfig_best: { label: "スピード指数（最良）" },
  asof_spdfig_recent3: { label: "スピード指数（直近3走）" },
  asof_spdfig_last: { label: "スピード指数（前走）" },
  asof_spdfig_count: { label: "スピード指数の対象走数" },
  // --- past market support (069 F02, as-of; s=log(q×N) of past races) ---
  asof_pm_support_last: { label: "過去走の市場支持（前走）" },
  asof_pm_support_mean3: { label: "過去走の市場支持（直近3走平均）" },
  asof_pm_support_mean5: { label: "過去走の市場支持（直近5走平均）" },
  asof_pm_support_best5: { label: "過去走の市場支持（直近5走最良）" },
  asof_pm_support_career: { label: "過去走の市場支持（通算平均）" },
  asof_pm_support_trend: { label: "市場支持の傾向（直近3走）" },
  asof_pm_support_sd5: { label: "市場支持のばらつき（直近5走）" },
  asof_pm_obs_count: { label: "市場支持の観測走数" },
  asof_pm_has_obs: { label: "市場支持の観測あり" },
};
