"""Shared fixtures: migrated PostgreSQL testcontainer + ORM session + FastAPI TestClient.

The app's lifespan builds its engine from DATABASE_URL (set by the `database_url` fixture), so the
TestClient reads the SAME database that the `session` fixture seeds (commit-visible across engines).
"""
